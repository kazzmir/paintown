...
bool moveDirection(double &x1, double &y1, double x2, double y2, double velocity)
{
	double angle;
...
class position
{
	...
	bool checkCollision(position *obj)
	{
 ...
 */
void do_circle(const Graphics::Bitmap & bmp, int x, int y, int radius, int d, void (*proc)(const Graphics::Bitmap &, int, int, int))
{
   int cx = 0;
   int cy = radius;
   int df = 1 - radius;
...
class mTimer
{
	...
	int unitConverter(int unit, int s)
	{
		switch(unit)
		{
			case (TIMERSECONDS):
	...
	public:
	/* Call initially to start the process Do not call twice!
	  * Query the initial Class to get it's current time
	  * Create new count down timers via the next constructor
	*/
	mTimer()
	{
		counter=0;
	...
	bool expired()
	{
		if(counter>=expireTime)return true;
		return false;
	}
	int getElapsed()
	{
	...
	int getRemaining()
	{
		return (expireTime - counter);
	}
	int getTotalTime(int unit)
	{
		return unitConverter(unit,counter);
	}
	void reset()
	{
		counter=0;
	...
	void print(int type, int x, int y, int color, const Graphics::Bitmap & dest, string msg="")
	{
		switch(type)
		{
			case (TOTAL):
...
class explosionGroup : public object
{
	...
	explosionGroup(int setID, double setx, double sety, double setVelocity, double setSpeed, double setRadius, bool setCollide=true, const Graphics::Bitmap & setBmp = NULL)
	{
		...
		{
			if((rand()%4)>2)particles.push_back(new particle(particles.size(),x,y,circlePoints[i].otherX,circlePoints[i].otherY,maxVelocity,speedIncrement,0,5,false,false,collidable,bmp));
			
			if((rand()%4)>2&&!toggleGfxThrottle)particles.push_back(new particle(particles.size(),x,y,circlePoints[i].otherX,circlePoints[i].otherY,maxVelocity,speedIncrement,15,5,false,false,collidable,bmp));
			
			if((rand()%4)>2&&!toggleGfxThrottle)particles.push_back(new particle(particles.size(),x,y,circlePoints[i].otherX,circlePoints[i].otherY,maxVelocity,speedIncrement,30,5,false,false,collidable,bmp));
			
			if((rand()%4)>2&&!toggleGfxThrottle)particles.push_back(new particle(particles.size(),x,y,circlePoints[i].otherX,circlePoints[i].otherY,maxVelocity,speedIncrement,45,5,false,false,collidable,bmp));
			
			if((rand()%4)>2)particles.push_back(new particle(particles.size(),x,y,circlePoints[i].otherX,circlePoints[i].otherY,maxVelocity,speedIncrement,60,5,false,false,collidable,bmp));
...
class window : public object
{
	...
	window(int setID, double setx, double sety, double setWidth, double setHeight, int setColor = BLACK, const Graphics::Bitmap & setBmp = NULL,bool setButton=false, string setText="nothing")
	{
		ID = setID;
		x = setx;
		y = sety;
...
class Manager
{
	...
	{
		map<int, Graphics::Bitmap>::iterator p;
	...
	void checkGameOver()
	{
		...
		if(gameOverChecker==cities.size()&&money==0)
		{
			paused=true;
			gameOver=true;
			//gameTime->pause(true);
		...
		else if(gameOverChecker==cities.size())
		{
	...
	Manager()
	{
		...
		//! Menu Related
		openMenuButton = new window(0, 100, 462, 40, 15, BLUE, NULL,true,"Shop");
		toggleMoamButton = new window(0, 280, 462, 80, 15, BLUE, NULL,true,"Open Base");
		launchMoamButton = new window(0, 365, 462, 50, 15, RED, NULL, true, "LAUNCH");
		
		windows.push_back(new window(windows.size(),120,50,400,350,BLACK,NULL,false));
		windows.push_back(new window(windows.size(),290,375,60,20,BLUE,NULL,true,"Resume"));
		windows.push_back(new window(windows.size(),440,100,60,15,BLUE,NULL,true,"Add"));
		windows.push_back(new window(windows.size(),440,120,60,15,BLUE,NULL,true,"Add"));
		windows.push_back(new window(windows.size(),440,160,60,15,BLUE,NULL,true,"Buy"));
		windows.push_back(new window(windows.size(),440,180,60,15,BLUE,NULL,true,"Buy"));
		windows.push_back(new window(windows.size(),440,200,60,15,BLUE,NULL,true,"Fix"));
		windows.push_back(new window(windows.size(),440,240,60,15,BLUE,NULL,true,"Buy"));
	...
	void doLogic(int mouse_x, int mouse_y)
	{
		...
		if(effects.size()<maxGfxThrottle)toggleGfxThrottle=false;
	...
	void doDrawing(const Graphics::Bitmap & dest)
	{
		...
		//gameTime->print(mTimer::TOTAL,522,471,WHITE,dest,"");
	...
	//! Game bitmaps
	map<int, Graphics::Bitmap>gameBitmaps;
...
class MissleDefend
{
	...
	void Draw(const Graphics::Bitmap & canvas) 
	{
		// Do all the drawing here. Draw only to the canvas bitmap.
		// ...
		
		gameManager->doDrawing(canvas);
		if(gameManager->inMenu)gameManager->doMenu(canvas);
		//if(!gameManager->paused)draw_sprite(canvas,gameManager->mouseCursor,mouse_x,mouse_y);
	}
	...
	int Logic(int keys[8], int mouse[4], float dT) 
	{
		...
		{
			if(!gameManager->inMenu)gameManager->doLogic(mouse_x, mouse_y);
			else gameManager->doMenuLogic(mouse_x, mouse_y);
		}
		if(gameManager->gameOver)return 1;
